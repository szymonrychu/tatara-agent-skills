...
2026-08-23 (#68, the vendored snapshot had no clock) **`.github/platform-vocabulary.json` was reproducible but not current, and the difference is a guard that rejects correct text.** #56 argued a hand-regenerated snapshot is safe because `gen_platform_vocabulary.py` derives it from the operator's source rather than transcribing it. One week later the operator had grown six park reasons (`merge-conflict`, `ci-pending`, `ci-failed`, `merge-conflict-retry`, `mr-surface-spent`, `retry-exhausted`), the snapshot still said v2.16.1, and `validate_vocabulary.py` was hard-failing skill text that MATCHED the operator - with `<!-- vocab-ok: -->` sitting right there as the wrong fix. Remedy is `tatara-helmfile#397`'s shape, not `#46`'s: a scheduled non-blocking `vocabulary-currency` job that re-derives from operator `main` and opens a bump PR, because the operator still publishes no GitHub Release assets so the tool-manifest fetch-and-pin design still does not transfer. **The generalisable rule: "derived by a script" is a claim about PROVENANCE, not about FRESHNESS. Any pin no CD writes needs something that can say how old it is, or its lag is unbounded and invisible.**

2026-08-23 (#68, two fail-open traps in a guard whose whole argument is fail-closed) Both found by adversarial review, both would have made the new job green or wrongly-green in exactly its motivating case. (1) **`gen_platform_vocabulary.die()` is `sys.exit(1)`, and 1 is the DRIFTED exit code.** Nearly every collapse the shrink guard exists to catch - a renamed operator source file, `var ParkReasons` moving out from under `SLICE_RE`, a `ROOT_STRUCT` rename - dies rather than deriving a short list, so an uncaught `SystemExit` sent the job down the propose-a-bump branch for the one scenario SUSPECT was written for. `build()` is now called inside a catch-all that maps any failure to SUSPECT. (2) **`set +e` to capture an exit code makes every unhandled status green.** Branching on `verdict == '1'` and `'2'` left 127 (broken PATH), 137 (OOM) and 143 (runner timeout) with the step at exit 0; a `case` now reds anything that is not a verdict. Also: an aggregate shrink threshold must sit BELOW the per-leaf one or it is arithmetic rather than a check - if the total loses more than half, some single leaf must have too.

2026-08-23 (#68, review round 2: a no-op guard measured the wrong thing) **`git diff --quiet` on the snapshot file cannot decide whether there is anything to push, because `compare()` and `git diff` disagree about what counts.** `gen_platform_vocabulary.py` stamps `provenance.commit` with the operator's `HEAD`, and `check_vocabulary_currency.py` deliberately excludes `provenance` from the comparison - so on every day the operator merged anything, the byte-level guard saw a diff in a field that is not drift and force-pushed a provenance-only commit onto the open bump PR, resetting its checks and any review. Under push-CD the day the operator does NOT move is the rare one, and a human-gated PR is open for exactly the days it does. The guard now re-runs `check_vocabulary_currency.py` after `checkout -B` (so it reads the BRANCH's snapshot) and skips on verdict 0, reusing the one comparator that already knows provenance is not drift. Second-order, and the reason the skip path no longer short-circuits: it used to `exit 1` before `gh pr edit`, so once the guard fired the "Do not merge as-is" DIRTY banner could never clear after a human fixed the skill text on the branch. **The generalisable rule: when a script already defines what "changed" means, a second definition in the shell around it is a bug waiting for the two to disagree - and they disagree first in the field the script was written to ignore.**
